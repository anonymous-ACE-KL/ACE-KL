...
class AceExplainer(object):
    ...
    def fit(self, train_X, train_y, train_sample_weights):
        ...
        while diff > 0.0001:
            ...
            """
            optimizer.zero_grad()
            train_loss = self.easy_model.cal_loss(X, y, sample_weights)
            # print('loss {}'.format(train_loss.data[0]))
            train_loss.backward()
            optimizer.step()

            diff = np.abs(prev_train_loss - train_loss.cpu().data.numpy()[0])
            prev_train_loss = train_loss.cpu().data.numpy()[0]
            # print "diff:", diff
            '''
            if epoch % 1000 == 0:
    ...
    def get_explanation(self,
        ...
        if verbose:
            ...
            x = torch.autograd.Variable(torch.from_numpy(original_row_data))
            if torch.cuda.is_available():
                x = x.cuda()
    ...
    def explain_instance(self,
        ...
        original_datum = data_row_neighbours_X[0]
        # pdb.set_trace()
